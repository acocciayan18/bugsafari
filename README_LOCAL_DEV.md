...
BugSafari cannot test `http://localhost:5173`. The Playwright engine runs inside a
container, so a loopback address resolves to the engine itself, never to your machine.
The target URL is used **exactly as entered** — there is no rewrite, no
`host.docker.internal` substitution, and no environment switch that changes this.

Loopback and private-network targets are rejected up front:

- The dashboard disables **Start Testing**, marks the URL field invalid, and shows an
  informational banner explaining the requirement.
- `POST /api/start-test` answers `422` with `TARGET_NOT_PUBLIC` for any client that
  submits one anyway (the worker re-checks before launching a job).

To test a local dev server, give it a publicly reachable address first — deploy it, or
expose it through a secure public tunnel — then enter the public URL you get back. Bind
the dev server to `0.0.0.0` rather than `127.0.0.1` so the tunnel can reach it (Vite:
`npm run dev -- --host 0.0.0.0`; Next.js: `next dev -H 0.0.0.0`).
