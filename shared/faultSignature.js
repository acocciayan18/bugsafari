...
export function normalizeFaultText(text) {
    return (text ?? '')
        .toLowerCase()
...
export function faultStackTop(stackTrace) {
    if (!stackTrace)
        return '';
    for (const line of stackTrace.split('\n')) {
        const trimmed = line.trim();
        if (trimmed)
            return normalizeFaultText(trimmed);
    }
    return '';
}
...
export function buildFaultSignature(fault) {
    return [
