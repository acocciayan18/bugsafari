...
export const VERBOSE_ACTION_CODES = new Set([
    'element-selected',
    'dom-elements-parsed',
    'dom-state-hash',
    'system-status',
...
const PRESERVED_TYPES = new Set(['EXCEPTION', 'BUG', 'NETWORK']);
// Idempotent lifecycle handshakes — one per run is enough (repeat IDLEs are noise).
function isOnceOnly(event) {
...
export function isVerboseTelemetry(event) {
...
export function telemetryDedupeKey(event) {
...
export function createTelemetryDeduper() {
    let lastKey = null;
    const seenOnceKeys = new Set();
    return {
        accept(event) {
