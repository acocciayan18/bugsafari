...
export const PUBLIC_TARGET_REQUIRED_MESSAGE = 'BugSafari can only test websites that are publicly reachable over the internet. Local addresses such as localhost, 127.0.0.1, or a private network IP cannot be opened by the testing engine. Deploy the site, or expose your development server through a secure public tunnel, and enter that public address instead.';
...
export function isPrivateTargetHost(hostname) {
    ...
    if (/^f[cd][0-9a-f]{2}:/.test(host) || /^fe80:/.test(host))
...
export function isPrivateTargetUrl(raw) {
