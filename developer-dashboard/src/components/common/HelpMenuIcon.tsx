...
function AccordionSection({ title, isOpen, onToggle, children }: AccordionSectionProps) {
  return (
    <div className="border-b border-(--border-hairline) last:border-b-0">
      <button
        onClick={onToggle}
        className="flex w-full items-center justify-between px-4 py-3 text-left text-sm font-medium text-(--text-secondary) hover:bg-(--surface-hover) transition-colors duration-200 ease-in-out"
        aria-expanded={isOpen}
      >
        <span>{title}</span>
        <svg
          className={`h-5 w-5 text-(--text-tertiary) transition-transform duration-200 ${isOpen ? 'rotate-180' : ''}`}
          fill="none"
          viewBox="0 0 24 24"
          stroke="currentColor"
          strokeWidth="1.5"
  ...
  return (
    <div ref={menuRef} className="relative">
      ...
      <button
        ...
        className="flex h-11 w-11 items-center justify-center rounded-full transition-colors duration-200 ease-in-out hover:bg-(--surface-hover) focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-(--border-focus) focus-visible:ring-offset-2"
      ...
      {isOpen && (
        <div
          className="absolute right-0 top-full z-50 mt-2 w-80 rounded-lg border border-(--border-hairline) bg-(--surface-panel) py-2 shadow-lg max-h-[70vh] overflow-y-auto"
          role="menu"
        >
          {/* Menu Header */}
            ...
            <div className="space-y-2 mt-2">
              {documentationLinks.map((link) => (
                <a
                  ...
                  className="flex items-start gap-3 p-2 rounded-md hover:bg-(--surface-hover) transition-colors duration-200 ease-in-out"
                >
                  <svg className="h-5 w-5 text-(--text-tertiary) mt-0.5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="1.5">
          ...
          <button
            ...
            className="flex w-full items-center gap-3 px-4 py-2.5 text-left text-sm text-(--text-secondary) hover:bg-(--surface-hover) transition-colors duration-200 ease-in-out"
            role="menuitem"
          >
