        ...
        {incidents.length === 0 ? (
          ...
          incidents.map((incident, index) => (
            <article key={`${incident.timestamp}-${index}`} className="mb-4 rounded-lg border border-(--status-critical-border) bg-(--status-critical-bg) p-3">
              ...
              <div className="text-xs text-(--status-critical-fg)">{new Date(incident.timestamp).toLocaleString()}</div>
              <div className="mt-1 text-xs text-(--text-secondary)">URL: {incident.url}</div>
