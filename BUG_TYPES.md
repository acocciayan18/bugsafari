...
| Malformed script / syntax error | Unparseable JS or JSON (build/template) | `SyntaxError: Unexpected end of input` | Message pattern match |
| Unhandled response exception / API contract violation | `.json()` on an HTML/non-JSON response — the endpoint returned an error/proxy page where JSON was expected. Distinct bug class `API_CONTRACT_VIOLATION` (CWE-754), not `RUNTIME_STABILITY_EXCEPTION` | `SyntaxError: Unexpected token '<', "<!DOCTYPE "... is not valid JSON` | JSON-contract signature match (`FaultClassifier` `API_CONTRACT` signal + runtime subtype) — outranks the active scenario's default class |
...
| Name | Description | Typical example | Detection |
|---|---|---|---|
| Client-side constraint bypass | Validation enforced only in the browser | Stripping `required`/`maxlength`/`disabled` still submits | `constraintBypassFinder` strips constraint attributes and submits; a correlated, same-origin, state-changing 2xx means the server did not re-validate. Suppressed while a rapid-click/concurrency burst (`ButtonSpammer`/`CoordinateBombing`/`AsyncStateRacer`/`ConcurrentClicker`) is active, so a burst-provoked autosave 2xx is not mislabeled a bypass. (`formBypasser.ts` is the strip-only scenario that mutates the DOM — it emits telemetry, not the finding.) |
