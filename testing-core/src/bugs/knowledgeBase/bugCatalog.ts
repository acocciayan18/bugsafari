...
export const BUG_CATALOG: Record<BugClass, BugDefinition> = {
  ...
  BOUNDARY_STRESS_FAILURE: {
    title: 'The app broke under a slow or failing network',
    description: 'The app failed when a request was delayed, cancelled, or returned an error status (a 5xx, a timeout, or a freeze).',
    defaultSeverity: 'HIGH',
    cwe: 'CWE-400',
    remediation: remediation(
      'Suggested fix: handle slow and failing requests',
      '1. Check that the endpoint is healthy and see what the failing call returns',
      '2. Add retry with backoff plus a visible error and timeout state for the user',
      '3. Guard the call site against empty, slow, or cancelled replies',
    ),
  },
