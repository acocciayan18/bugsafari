...
// On-demand LLM remediation via Google Gemini. Best-effort: any missing key,
// timeout, transport error, or empty output resolves to null so the caller can
// fall back to the deterministic knowledge-base advice.
...
function buildPrompt(req: SuggestFixRequest): string {
  const facts = [
